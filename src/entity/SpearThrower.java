...
public class SpearThrower extends Enemy {
    ...
    public void attack(int targetX, int targetY) {
        if (this.canAttack) {
            Spear spear = new Spear(this.getX(), this.getY(), 20, this.getDamage());
    ...
    /**
     * Aktualizuje všetky oštepy – pohne nimi, alebo ich odstráni, ak sú neaktívne.
     */
    public void updateSpears() {
        List<Spear> toRemove = new ArrayList<>();
        for (Spear spear : this.spears) {
            if (spear.isActive()) {
                spear.updateSpear();
            } else {
                toRemove.add(spear);
        ...
        this.spears.removeAll(toRemove);
    }
    /**
     * @return Zoznam všetkých aktívnych oštepov vystrelených týmto nepriateľom.
     */
